...
public class ParseXMLUtil {
     ...
     */
    public static com.mod.loan.util.kuaiqian.util.ParseXMLUtil initParseXMLUtil() {
        return new com.mod.loan.util.kuaiqian.util.ParseXMLUtil();
    }
    ...
    private HashMap parseXMLNode(HashMap hm, Element e) {
        ...
        for (Object o : e.getChildren()) {
            //获取节点下的每一个子元素
            child = (Element) o;
            //将每一个元素的名称和值都保存到HashMap中，方便以后查询取出
        ...
        for (Iterator childs = e.getChildren().iterator(); childs.hasNext(); ) {
            ...
            if (flag2.equals(child.getName())) {
                ...
                for (Iterator childs2 = child.getChildren().iterator(); childs2.hasNext(); ) {
                    //重新实例化一个HashMap对象，用来保存Txn中的元素
                    HM_txn = new HashMap();
                    //获取每一个子元素
                    child_txn = (Element) childs2.next();
                    //调用parseXMLNode函数，得到该节点下的所有元素
                    HM_txn = parseXMLNode(HM_txn, child_txn);
                    //将HM_txn放入List列表中，方便以后查询取出
                    list.add(HM_txn);
                }
                hm.put("Txn", list);
            } else {
